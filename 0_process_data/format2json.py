    ...
    for lang in langs:
        ...
        for file_type in file_types:
            ...
            for idx in range(0, l):
                ...
                if False and file_type == 'train':
                    tree = parser.parse(doc_token[4].encode('utf-8'))
                    traverse_identifiers(tree.root_node)
                    code_temp = str(tree.root_node.text, 'utf-8')
                    global  var_names
                    var_names = list(set(var_names))
                    for var_idx, var_name in enumerate(var_names):
                        code_temp = code_temp.replace(str(var_name, 'utf-8'), "<extra_id_" + str(var_idx) + ">")
                    var_names = []

                    doc_token[4] = code_temp

                print(idx)
                print(doc_token[4])
                code_md5 = md5_string(doc_token[4])
                review_md5 = md5_string(doc_token[3])
                _id_code = 'c_' + str(idx)
                _id_query = 'q_' + str(idx)
                _id_code = code_md5
                _id_query = review_md5
                code_dic.update({_id_code: doc_token[4]})
                review_dic.update({_id_query: doc_token[3]})

                #过滤相同样本
                id_md5 = md5_string( doc_token[4] + '**' + doc_token[3])
            ...
            if file_type != 'test': # 这里只取test 之前是train
                continue
