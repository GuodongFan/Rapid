...
tokenizer = RobertaTokenizer.from_pretrained("/data1/fgd/workplace/models/CDCS/")
model = RobertaForSequenceClassification.from_pretrained("/data1/fgd/workplace/models/CDCS/")
model = model.to(device)
model.eval()
# 为了测试时间
start_time = time.time()
file = open("Dataset/advtest/repo_test.txt", "r")
...
for idx, line in enumerate(tqdm(data_lines)):
    fields = line.split('<CODESPLIT>')

    code_content = fields[4]

    md5_hash = hashlib.md5()
    # 更新哈希对象以包含字符串数据
    md5_hash.update(code_content.encode())
...
for idx, line in enumerate(tqdm(data_lines[:])):
    ...
    real_methods_list = [code_path+'#'+code_name]
...
for review_dic, real_methods_list, sort_id in zip(reviews, query_method_list, sort_id_list):
    ...
    for method in real_methods_list:
        ...
        new_real_file_set.add(method.split('#')[0])
    ...
    for idx in sort_id[:1000]:
        ...
        if find is False:
            rank +=1
        if method_path in new_real_method_list:
            print(rank)
            find = True
            ranks.append(1/rank)
            break
    ...
    for i in sort_id:
        ...
        if method_path in visited_method_set:
            continue
        pre_methods_list.append(method_path)
        code_content_list.append(code_content)
        visited_method_set.add(method_path)
...
for real_methods_list, sort_id in zip(query_method_list, sort_id_list):
    ...
    for method_id in real_methods_list:
        real_code_path_list.append(method_id)
